<template>
    ...
    <div class="pageContent">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane :label="item.tag" :name="item.tag" v-for="(item,index) in typeList" :key="index"></el-tab-pane>
      </el-tabs>
      <div class="listBox" v-if="total>0"> 
        <el-card :body-style="{ padding: '10px' }" shadow="hover" v-for="(item,index) in blogData" :key="index" class="block">
          <el-image :src="imgHost+item.url" fit="fill" @click="handleInfo(item.id)">
...
<script>
import {listweb,getBlogClassity} from "@/api/blog"
