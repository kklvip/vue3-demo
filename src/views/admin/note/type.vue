<template>
    <div class="app-container calendar-list-container">
      ...
      <el-table @selection-change="collectId" :data="noteData" v-loading="listLoading" element-loading-text="努力加载中..." border fit highlight-current-row style="width: 100%">
        <el-table-column
          type="selection"
          align="center"
          width="50">
  ...
  <script>
    ...
    import {getNoteClassityList,updateClassityStatus,classitySave,deleteNoteClassity,getNoteById2} from "@/api/note"
    import { ElMessage,ElMessageBox } from 'element-plus'
    import { rules } from "@/utils/validate"
    export default {
        name:'adminNoteClassity',
        setup(){
            ...
            function getList(){
                ...
                getNoteClassityList(listQuery).then(res=>{
            ...
            function handleDelete(id){
                getNoteById2(id).then(response=>{
                    if(response.data.code=="200"){
                        if(response.data.data.length>0){
                            ElMessageBox.confirm('该分类已经被笔记：'+response.data.data[0].title+' 使用， 确认要删除吗?','提示',
                                {
                                    ...
                                    if(id){
                                        deleteNoteClassity(id).then(res=>{
                                            if(res.data.code=="200"){
