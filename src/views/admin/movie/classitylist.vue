...
<script>
    ...
    import {movieClassityFindpage,updateClassityStatus,saveClassity,deleteClassity} from "@/api/movie"
    import { ElMessage,ElMessageBox } from 'element-plus'
    import { rules } from "@/utils/validate"
    export default {
        name:'adminMovieClassity',
        setup(){
            ...
            function getList(){
                ...
                movieClassityFindpage(listQuery).then(res=>{
                    if(res.data.code=="200"){
                        movieData.length = 0;
                        listLoading.value = false;
                        total.value = res.data.data.length
                        movieData.push(...res.data.data)
                    }else{
                        listLoading.value = false;
                    }
                }).catch(error=>{
