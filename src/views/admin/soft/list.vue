<template>
    <div class="app-container calendar-list-container">
      ...
      <el-table @selection-change="collectId" :data="softData" v-loading="listLoading" element-loading-text="努力加载中..." border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column property="tag" label="分类" />
        <el-table-column property="createTime" label="创建时间" />
        <el-table-column property="updateTime" label="修改时间" />
        <el-table-column property="views" label="浏览量" />
        <el-table-column  label="状态" >
            <template #default="scope">
            <el-switch v-model="scope.row.status" @change="changesoftStatus(scope.row.id,scope.row.status)"></el-switch>
            </template>
        </el-table-column>
  ...
  import {listadmin,changeStatus,deleteSoft} from "@/api/soft"
  ...
  export default {
    ...
    setup(){
      ...
      function getList(){
        ...
        listadmin(listQuery).then(res=>{
          if(res.data.code=="200"){
              ...
              softData.push(...res.data.data.list)
          }
      ...
      function handleAdd(){
        store.state.softId = '';
        router.push("/soft/add")
      }
      function handleEdit(id){
        if(id){
          store.state.softId = id;
          router.push("/soft/edit")
        }
      }
      ...
      function changesoftStatus(id,status){
