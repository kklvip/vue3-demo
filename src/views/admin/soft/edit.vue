<template>
    <div class="app-container calendar-list-container">
      <h1>{{softId?'soft编辑':'soft添加'}}</h1>
      <h2>{{}}</h2>
      <div class="panelBody" >
        <el-form class="dataForm" style="width:780px;margin:0 auto" label-position="right" label-width="80px" ref="dataForm" :model="temp">
          ...
          <el-form-item label="图片:" :rules="{required:true,message:'请上传图片',trigger:'change'}">
            <UploadFile paramType="3" @success="handleImgSuccess" :path="temp.pic"></UploadFile>
          </el-form-item>
          <el-form-item label="软件:" :rules="{required:true,message:'请上传软件',trigger:'change'}">
            <UploadFile1 paramType="4" @success="handleImgSuccess2" @remove="handleRemove()"></UploadFile1>
          </el-form-item>
          <el-form-item label="摘要:" prop="summary">
            <el-input v-model="temp.summary" placeholder="请输入摘要"></el-input>
          </el-form-item>
          <el-form-item label="版本号:" prop="version" :rules="{required:true,message:'请输入版本号',trigger:'change'}">
  ...
  <script>
    ...
    import UploadFile1 from '@/components/UploadFile/file'
    import {imgHost} from "@/utils/config"
    import {save,getSoftById,listclassityweb } from "@/api/soft"
    import { Editor, Toolbar } from '@wangeditor/editor-for-vue'
    import '@wangeditor/editor/dist/css/style.css' // 引入 css
    export default{
      name:'softEdit',
      components: { UploadFile,UploadFile1,Editor,Toolbar },
      setup(){
        ...
        onMounted(()=>{
            if(store.state.softId){
                softId.value = store.state.softId
            }else{
                softId.value = ""
            }
            if(softId.value){
        ...
        function  getInfo(){
            getSoftById(softId.value).then(res=>{
        ...
        function saveData(){
            dataForm.value.validate().then(()=>{
              if(!temp.value.pic){
                ElMessage({message: '软件图片还没有上传',type: 'warning'})
              }else if(!temp.value.url){
                ElMessage({message: '软件还没有上传',type: 'warning'})
              }else{
        ...
        function handleImgSuccess2(val){
          temp.value.fileSize = val.data.fileSize
