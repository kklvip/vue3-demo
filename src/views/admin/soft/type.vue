<template>
    <div class="app-container calendar-list-container">
      ...
      <el-table @selection-change="collectId" :data="softData" v-loading="listLoading" element-loading-text="努力加载中..." border fit highlight-current-row style="width: 100%">
        <el-table-column
          type="selection"
          align="center"
          width="50">
  ...
  <script>
    ...
    import {classityFindPage,updateClassityStatus,saveClassity,deleteClassity,getSoftById2} from "@/api/soft"
    import { ElMessage,ElMessageBox } from 'element-plus'
    import { rules } from "@/utils/validate"
    export default {
        name:'adminsoftClassity',
        setup(){
            let softData = reactive([])
            ...
            function handleDelete(id){
                getSoftById2(id).then(response=>{
                    if(response.data.code=="200"){
                        if(response.data.data.length>0){
                            ElMessageBox.confirm('该分类已经被软件：'+response.data.data[0].title+' 使用， 确认要删除吗?','提示',
                                {
            ...
            const saveData = ()=>{
                actionForm.value.validate().then(()=>{
                    saveClassity(updateData.value).then(res=>{
                        if(res.data.code=="200"){
